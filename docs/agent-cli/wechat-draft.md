...
>
> **预估阅读**: 6 分钟  ·  **配图建议**: 1) `llms.txt` 截图 2) `npx zhanglu list skills` 终端截图 3) 端点架构图

---

## 标题候选

1. **给 agent 写了个 CLI：用一行 `npx zhanglu` 替代 HTML 抓取**
...
**CLI 零运行时依赖**。Node 18+ 内置的 `fetch` 替代 `node-fetch`，`node:util` 的 `parseArgs` 替代 `commander`，ANSI escape codes 手拼替代 `chalk`。`npx zhanglu` 启动快，agent 调也不卡。
...
**Claude Code 里**：我写了个 `/zhanglu` skill，触发词包括「查张路的 skill」「zhanglu 上的 X」。skill 会自动调 `npx zhanglu`。
...
```bash
npx zhanglu list skills --featured --json
npx zhanglu get skill mba --md
npx zhanglu search "品牌"
```
